...
bool DriverComm::InjectDllWithNoModuleByEventHook(IN DWORD pid, IN LPCWSTR dllPath)
{
    if (!is_init_)
    {
        LOG("no init");
    ...
    ZeroMemory(&cmsg_, sizeof(COMM::CMSG));
    cmsg_.oper = COMM::Operation::Oper_InjectDllWithNoModuleByEventHook;
    cmsg_.needOutput = false;
    cmsg_.input_InjectDllWithNoModuleByEventHook.pid = pid;
    wcscpy_s(
        cmsg_.input_InjectDllWithNoModuleByEventHook.dllPath,
        sizeof(cmsg_.input_InjectDllWithNoModuleByEventHook.dllPath) / sizeof(WCHAR),
        dllPath
    );
...
bool DriverComm::InitVm()
{
    ...
    ZeroMemory(&cmsg_, sizeof(COMM::CMSG));
    cmsg_.oper = COMM::Operation::Oper_InitVm;
    cmsg_.needOutput = false;
...
bool DriverComm::UnInitVm()
{
    ...
    ZeroMemory(&cmsg_, sizeof(COMM::CMSG));
    cmsg_.oper = COMM::Operation::Oper_UnInitVm;
    cmsg_.needOutput = false;
