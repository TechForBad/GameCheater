...
PVOID GetKernelBase(OUT PULONG pSize)
{
    ...
    if (pMods)
    {
        ExFreePoolWithTag(pMods, 0x504D5448);
    }
...
LONG RetrieveMmUnloadedDriversData(VOID)
{
    ...
    status = ZwQuerySystemInformation(SystemModuleInformation, processModules, bytes, &bytes);
    if (!NT_SUCCESS(status))
    {
        return 2;
    }
    ...
    {
        if (0 == strcmp((char*)modules[i].FullPathName, "\\SystemRoot\\system32\\ntoskrnl.exe"))
        {
            ntoskrnlBase = (UINT64)modules[i].ImageBase;
            ntoskrnlSize = (UINT64)modules[i].ImageSize;
            break;
        }
    }
    if (modules)
    ...
    if (NULL == MmUnloadedDriversInstr)
    {
        return 3;
    }
    ...
    if (NULL == MmLastUnloadedDriverInstr)
    {
        return 4;
    }
