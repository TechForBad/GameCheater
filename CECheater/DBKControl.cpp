...
// 获取驱动地址
PVOID GetDriverAddressByName(LPCWSTR driverName)
{
...
{
	// 获取驱动文件全路径
	wchar_t driverFilePath[MAX_PATH] = { 0 };
	if (!tool::GetCurrentModuleDirPath(driverFilePath))
	{
	...
	// 写相关注册表
	HKEY hKey;
	std::wstring subKey = tool::Format(L"SYSTEM\\CurrentControlSet\\Services\\%ws", DBK_SERVICE_NAME);
...
{
	// 打开设备对象
	std::wstring deviceName = tool::Format(L"\\\\.\\%ws", DBK_SERVICE_NAME);
